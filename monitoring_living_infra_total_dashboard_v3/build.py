#!/usr/bin/env python3
"""
build.py — 생활인프라 편리성 GIS 분석 결과 → 정적 HTML 대시보드 v3

사용법:  python build.py
출력:    dashboard.html  (브라우저에서 바로 열 수 있는 단일 파일)

v3 변경사항:
  - 최종 점수(infra_idx)가 0~100 리스케일 방식에서
    부문별 T점수 편리성 지수의 가중 평균(합산) 방식으로 변경
  - 모든 세부지수(_sup, _pop, _acc)도 minmax → T점수 표준화로 변경
  - 점수 표시 소수점 통일: f3 → f1 (T점수 스케일 반영)
  - 종합지수 색상 범위: 하드코딩 0~100 → 데이터 기반 동적 범위
"""
...
def load():
    ...
    # v3: 종합지수를 0~100 rescale 값 대신 부문별 T점수 평균(infra_avg)으로 교체
    if 'infra_avg' in comp.columns:
        comp['infra_idx'] = comp['infra_avg']
    elif 'infra_raw' in comp.columns:
        comp['infra_idx'] = comp['infra_raw']
...
<title>생활인프라 편리성 모니터링 대시보드 v3</title>
...
/* Ranking tab */
.rank-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; align-items: stretch; }
@media(max-width:900px){ .rank-layout { grid-template-columns: 1fr; } }
.rank-layout > * { display: flex; flex-direction: column; min-height: 0; height: 100%; box-sizing: border-box; }
...
.table-wrap { overflow-y: auto; border-radius: 8px; border: 1px solid #E2E8F0; flex: 1; min-height: 0; }
.rank-card-inner { display: flex; flex-direction: column; height: 100%; }
...
<div class="navbar">
  ...
  <span class="badge-cnt" style="background:#1e3a5f;color:#60A5FA">v3</span>
</div>
    ...
    <div>
      <div class="card card-p mb-3" style="padding-bottom:10px">
        ...
        <div style="display:flex;gap:5px;flex-wrap:wrap">
          <label style="display:flex;align-items:center;gap:4px;font-size:.8rem;cursor:pointer;padding:4px 8px;border-radius:6px;border:1px solid #E2E8F0;flex:1;justify-content:center;min-width:56px" id="lbl-equal">
            <input type="radio" name="classify" value="equal" checked style="display:none">
            <span>등간격</span>
          </label>
          <label style="display:flex;align-items:center;gap:4px;font-size:.8rem;cursor:pointer;padding:4px 8px;border-radius:6px;border:1px solid #E2E8F0;flex:1;justify-content:center;min-width:56px" id="lbl-quantile">
            <input type="radio" name="classify" value="quantile" style="display:none">
            <span>분위수</span>
          </label>
          <label style="display:flex;align-items:center;gap:4px;font-size:.8rem;cursor:pointer;padding:4px 8px;border-radius:6px;border:1px solid #E2E8F0;flex:1;justify-content:center;min-width:56px" id="lbl-decile">
            <input type="radio" name="classify" value="decile" style="display:none">
            <span>10등급</span>
          </label>
        </div>
        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:.8rem;margin-top:8px;color:#374151">
          <input type="checkbox" id="show-labels" style="cursor:pointer">
          <span>값 표시</span>
        </label>
      </div>
  ...
  <div class="rank-layout">
    ...
    <div class="cmp-layout">
      ...
      <div class="card card-p">
        ...
        <div id="cmp-radar" style="height:320px"></div>
      </div>
      <!-- 오른쪽: 비교 테이블 -->
      <div class="card card-p">
...
document.querySelectorAll('.tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    else if (tab === 'map' && leafMap)   { setTimeout(() => leafMap.invalidateSize(), 50); }
...
function renderOverview() {
  ...
  // 전국 평균에 가장 가까운 지역
  const avgRec = RECORDS.reduce((best, r) =>
    Math.abs((r.infra_idx || 0) - avg) < Math.abs((best.infra_idx || 0) - avg) ? r : best
  , RECORDS[0]);

  document.getElementById('data-badge').textContent = RECORDS.length + '개 시군구';

  // Summary stats: 분석시군구 | 최고지역 | 평균지역 | 최저지역
  document.getElementById('summary-stats').innerHTML = `
    ...
    <div class="stat-card">
      <div class="stat-label">평균 지역 ≈</div>
      <div class="stat-val" style="font-size:1rem">${avgRec.sido_nm_k} ${avgRec.sgg_nm_k}</div>
      <div class="stat-sub" style="color:#2563EB">${f1(avgRec.infra_idx)}점 (전국 평균 ${f1(avg)}점)</div>
    </div>
    <div class="stat-card">
  ...
  // Sector cards — 최저/최고 범위 + 대표 3개 지역(최고·평균·최저)
  document.getElementById('sector-cards').innerHTML = Object.entries(SEC).map(([k, s]) => {
    const recs = RECORDS.filter(r => r[k + '_conv'] != null);
    const vals = recs.map(r => r[k + '_conv']);
    const mn = Math.min(...vals), mx = Math.max(...vals);
    const mean = vals.reduce((a, b) => a + b, 0) / vals.length;
    const sortedRecs = [...recs].sort((a, b) => b[k + '_conv'] - a[k + '_conv']);
    const topRec = sortedRecs[0];
    const botRec = sortedRecs[sortedRecs.length - 1];
    const avgRec = recs.reduce((best, r) =>
      Math.abs((r[k + '_conv'] || 0) - mean) < Math.abs((best[k + '_conv'] || 0) - mean) ? r : best
    , recs[0]);
    return `<div class="sector-card">
      <span class="sector-chip" style="background:${s.color}">${s.label}</span>
      <div style="position:relative;height:8px;background:#F1F5F9;border-radius:4px;margin:10px 0 4px">
        <div style="position:absolute;inset:0;border-radius:4px;background:linear-gradient(to right,${s.color}44,${s.color})"></div>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:.7rem;color:#94A3B8;margin-bottom:8px">
        <span>최저 ${f1(mn)}</span><span>최고 ${f1(mx)}</span>
      </div>
      <div style="font-size:.7rem;border-top:1px solid #F1F5F9;padding-top:6px;display:flex;flex-direction:column;gap:4px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <span style="color:#94A3B8;white-space:nowrap">🥇 최고</span>
          <span style="text-align:right"><strong style="color:#1E293B">${topRec.sido_nm_k} ${topRec.sgg_nm_k}</strong> <span style="color:${s.color};font-weight:600">${f1(topRec[k + '_conv'])}</span></span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center">
          <span style="color:#94A3B8;white-space:nowrap">〜 평균</span>
          <span style="text-align:right"><strong style="color:#1E293B">${avgRec.sido_nm_k} ${avgRec.sgg_nm_k}</strong> <span style="color:#64748B;font-weight:600">${f1(avgRec[k + '_conv'])}</span></span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center">
          <span style="color:#94A3B8;white-space:nowrap">⬇ 최저</span>
          <span style="text-align:right"><strong style="color:#1E293B">${botRec.sido_nm_k} ${botRec.sgg_nm_k}</strong> <span style="color:#DC2626;font-weight:600">${f1(botRec[k + '_conv'])}</span></span>
        </div>
      </div>
    </div>`;
  }).join('');

  // Top / Bottom 10 — 종합(평균)과 종합(합계) 함께 표시
  function miniTable(rows) {
    return `<table class="mini-table">
      <thead><tr><th>#</th><th>시도</th><th>시군구</th><th>종합(평균)</th><th>종합(합계)</th></tr></thead>
      <tbody>${rows.slice(0, 10).map((r, i) => {
        ...
        return `<tr>
          ...
          <td style="color:#64748B">${f1(r.infra_sum)}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
  }
...
let curClassify = 'equal'; // 'equal' | 'quantile' | 'decile'
...
// 폴리곤 좌표의 산술 평균으로 시각적 중심 계산 (bounding box center보다 정확)
function getPolygonCentroid(ft) {
  const coords = [];
  const collect = arr => {
    if (!arr.length) return;
    if (typeof arr[0][0] === 'number') arr.forEach(c => coords.push(c));
    else arr.forEach(a => collect(a));
  };
  collect(ft.geometry.coordinates);
  if (!coords.length) return L.geoJSON(ft).getBounds().getCenter();
  return L.latLng(
...
function addLabels() {
  removeLabels();
  const features = curSidoFilter
    ? GEOJSON.features.filter(f => f.properties.sido_nm_k === curSidoFilter)
    : GEOJSON.features;
  features.forEach(ft => {
    const v = ft.properties[curMetric];
    if (v == null) return;
    const center = getPolygonCentroid(ft);
    const txt = curClassify === 'decile' ? `${getDecileGrade(v)}등급` : fAuto(v, curMetric);
    const bgColor = curClassify === 'decile'
      ? (decileColors[getDecileGrade(v) - 1] || '#334155') : '#334155';
    const m = L.marker(center, {
      icon: L.divIcon({
        className: '',
        html: `<div style="display:inline-block;font-size:10px;font-weight:700;color:#fff;background:${bgColor};padding:2px 5px;border-radius:4px;white-space:nowrap;box-shadow:0 1px 3px rgba(0,0,0,.35);transform:translate(-50%,-50%);pointer-events:none;line-height:1.4">${txt}</div>`,
        iconAnchor: [0, 0]
      }),
      interactive: false
    }).addTo(leafMap);
...
function removeLabels() {
  labelMarkers.forEach(m => { if (leafMap) leafMap.removeLayer(m); });
  labelMarkers = [];
}
...
function initMap() {
  ...
  function updateClassifyUI() {
    ['equal','quantile','decile'].forEach(v => {
      ...
      el.style.cssText += on ? ';background:#EFF6FF;color:#2563EB;border-color:#93C5FD;font-weight:600'
                             : ';background:#fff;color:#374151;border-color:#E2E8F0;font-weight:400';
    });
  }
  updateClassifyUI();
  ...
  document.getElementById('show-labels').addEventListener('change', e => {
    ...
    if (showLabels) addLabels(); else removeLabels();
  });
...
function renderChoropleth() {
  ...
  if (curClassify === 'quantile') {
    ...
    // 전국 10분위 등급 — 시도 필터 시에도 전국 기준 등급 유지
    const sorted = [...allVals].sort((a, b) => a - b);
    const n = sorted.length;
    decileBreaks = Array.from({length: 11}, (_, i) =>
      i < 10 ? sorted[Math.floor(i * n / 10)] : sorted[n - 1]
    );
    decileColors = chroma.scale(PALETTE).colors(10);
    // decileColors[0]=파랑(1등급·최고), decileColors[9]=빨강(10등급·최저)
  } else {
  ...
  function getFillColor(v) {
    ...
    if (curClassify === 'decile') return decileColors[getDecileGrade(v) - 1] || '#ccc';
    return scale(v).hex();
  }

  geoLayer = L.geoJSON(geoData, {
    style: ft => ({
      fillColor:   getFillColor(ft.properties[curMetric]),
      fillOpacity: 0.75,
      color: '#fff',
      weight: 0.6,
    }),
    onEachFeature: (ft, layer) => {
      ...
      const gradeStr = curClassify === 'decile'
        ? `<br>등급: <b>${getDecileGrade(p[curMetric])}등급</b>` : '';
      layer.bindTooltip(
        `<strong>${p.sido_nm_k} ${p.sgg_nm_k}</strong><br>${metricLabel}: <b>${fAuto(p[curMetric], curMetric)}</b>${gradeStr}`,
  ...
  legendCtrl.onAdd = () => {
    ...
    div.style.cssText = 'background:#fff;padding:10px 12px;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,.15);font-size:.75rem;min-width:140px;max-height:360px;overflow-y:auto';
    const metricLabel = METRIC_DEFS.find(m => m.key === curMetric)?.label || curMetric;
    let html = `<div style="font-weight:600;margin-bottom:6px;color:#374151">${metricLabel}</div>`;
    if (curClassify === 'decile') {
      for (let g = 1; g <= 10; g++) {
        ...
        html += `<div style="display:flex;align-items:center;gap:5px;margin-bottom:2px">
          <div style="width:13px;height:13px;border-radius:3px;flex-shrink:0;background:${decileColors[g-1]}"></div>
          <span>${g}등급 ${fAuto(lo, curMetric)}–${fAuto(hi, curMetric)}</span>
        </div>`;
      }
  ...
  document.getElementById('map-info-card').innerHTML = `
    ...
    <div style="display:flex;flex-wrap:wrap;gap:4px">
      ${Object.entries(SEC).map(([k, s]) =>
        `<span class="score-badge" style="background:${s.color}22;color:${s.color}">${s.label} ${f1(p[k + '_conv'])}</span>`
      ).join('')}
    </div>`;
}
...
function renderMapTable() {
  ...
  const filtered = curSidoFilter
    ? RECORDS.filter(r => r.sido_nm_k === curSidoFilter)
    : RECORDS;
  const sorted = filtered.filter(r => r[curMetric] != null)
    .sort((a, b) => b[curMetric] - a[curMetric]);
  const [mn, mx] = getRange(curMetric);
  const isDecile = curClassify === 'decile';
  const rows = sorted.slice(0, 20).map((r, i) => {
    const c = colorFor(r[curMetric], mn, mx);
    const grade = isDecile ? getDecileGrade(r[curMetric]) : null;
    const gradeCell = isDecile
      ? `<td><span style="color:#fff;background:${decileColors[grade-1]};font-weight:700;font-size:.75rem;padding:1px 6px;border-radius:10px">${grade}등급</span></td>`
      : '';
    return `<tr>
      <td style="color:#94A3B8">${i + 1}</td>
      <td>${r.sido_nm_k}</td>
  ...
  const gradeHeader = isDecile ? '<th>등급</th>' : '';
  document.getElementById('map-top20').innerHTML = `
    <table class="mini-table">
      <thead><tr><th>#</th><th>시도</th><th>시군구</th><th>${label}</th>${gradeHeader}<th>종합지수</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}
...
function renderComparison() {
  ...
  if (cmpMode === 'direct') {
    ...
    document.getElementById('cmp-similar-desc').textContent =
      `기준: ${base.sido_nm_k} ${base.sgg_nm_k} (${cmpMode === 'pop' ? '인구' : '면적'} ${baseVal}${unit}) 와 유사한 상위 4개 지역`;
  }
