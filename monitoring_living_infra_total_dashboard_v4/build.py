    ...
    <div class="map-right">
      ...
      <div class="card card-p" style="padding:10px 14px">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          <span id="detail-year-ctrl" style="display:none;align-items:center;gap:6px">
            <span class="year-seg-local" id="detail-map-year-seg" style="padding:2px"></span>
            <button id="change-view-btn" type="button" class="sido-btn">🔁 변화보기</button>
          </span>
...
// ── 충족격자(access) ─────────────────────────────────────────────────────────
let mapMode = 'compare';        // 'compare'(듀얼 연도 비교) | 'detail'(단일 시점 상세)
let detailMapYear = Y2;         // 상세보기 표시 시점
let changeView = false;         // 상세보기 변화보기(연도 비교) 토글
...
function updateSvcPanel() {
  ...
  const singleHdr = mapMode === 'detail' && !changeView ? ` — ${detailMapYear}년` : ` — ${Y1}→${Y2}`;
  let html = `<div class="leg-title" style="margin-bottom:2px">서비스권역 내 인구비율 (%)${singleHdr}</div>
  ...
  SECTOR_ORDER.forEach(sk => {
    FACILITIES.filter(f => f[2] === sk).forEach(f => {
      ...
      const single = mapMode === 'detail' && !changeView;
      const ry = detailMapYear === Y1 ? r1 : r2;
      const d = (r1 != null && r2 != null) ? r2 - r1 : null;
      const valueHtml = single
        ? `<b>${ry ?? '-'}</b>`
        : `<span style="color:var(--ink-3)">${r1 ?? '-'}</span> → <b>${r2 ?? '-'}</b> ${d == null ? '' : deltaHtml(d)}`;
      const barsHtml = single
        ? bar(ry, SEC[sk].color, .9)
        : bar(r1, '#94A3B8', .55) + bar(r2, SEC[sk].color, .9);
      html += `<div class="svc-row" data-fac="${f[0]}" title="${label}"
        ...
        <div style="display:flex;justify-content:space-between;align-items:baseline;gap:6px">
          ...
          <span style="white-space:nowrap;font-size:.68rem">${valueHtml}</span>
        </div>
        ${barsHtml}
      </div>`;
    });
...
function gridTooltipHtml(d) {
  // 단일 시점 모드: 해당 격자의 표시 연도 값만
  if (mapMode === 'detail' && !changeView) {
    const mk1 = bit => ((d.mask >> bit) & 1)
    ...
    SECTOR_ORDER.forEach(sk => {
      ...
      const items = FACILITIES.map((f, bit) => f[2] !== sk ? '' :
        `<div style="display:flex;justify-content:space-between;gap:6px;white-space:nowrap">
          <span>${f[1]}</span><span>${mk1(bit)}</span>
        </div>`).join('');
    ...
    return `<div style="font-size:11px;line-height:1.55;max-width:330px">
      <div style="font-weight:700;margin-bottom:3px">500m 격자 (${d.year}년)</div>
      <div>충족점수: <b>${d.score}</b>/20 · 격자 인구: <b>${(d.pop || 0).toLocaleString()}</b>명</div>
      ${secRows}
    </div>`;
  }
...
function mapTooltipFor(year) {
  return ({ object }) => {
    ...
    if (mapMode === 'detail' && !changeView) {
      return {
        html: `<b>${p.sido_nm_k} ${p.sgg_nm_k}</b><br>${ml}<br>${year}: <b>${fAuto(v, curMetric)}</b>${gradeStr}`,
...
function initMap() {
  ...
  const detailYearCtrl = document.getElementById('detail-year-ctrl');
  const detailYearSeg = document.getElementById('detail-map-year-seg');
  const changeViewBtn = document.getElementById('change-view-btn');
  function renderDetailYearSeg() {
    detailYearSeg.innerHTML = YEAR_LIST.map(y =>
      `<button class="${y === detailMapYear ? 'active' : ''}" data-y="${y}" style="padding:3px 11px;font-size:.76rem">${y}</button>`).join('');
  }
  function updateDetailCtrlUI() {
    detailYearCtrl.style.display = mapMode === 'detail' ? 'inline-flex' : 'none';
    renderDetailYearSeg();
    chipStyle(changeViewBtn, changeView);
    document.getElementById('map-chip-y2').textContent =
      mapMode === 'detail' ? detailMapYear + '년 · 상세보기' + (changeView ? ' (변화보기)' : '') : Y2 + '년';
  }
  detailYearSeg.addEventListener('click', e => {
    const b = e.target.closest('button');
    if (!b || b.dataset.y === detailMapYear) return;
    detailMapYear = b.dataset.y;
    updateDetailCtrlUI();
    refreshOverlayLayers();
  });
  ...
  deckMap = new deck.DeckGL({
    ...
    controller: true,
    layers: [],
    getTooltip: obj => mapTooltipFor(mapMode === 'detail' ? detailMapYear : Y2)(obj),
...
function renderChoropleth() {
  ...
  deckMap.setProps({ layers: buildMapLayers(mapMode === 'detail' ? detailMapYear : Y2) });
...
function overlayLegendHtml() {
  ...
  const e = facEntry(layerFac);
  const posLabel = mapMode === 'detail' ? `${e[1]} 위치 (${detailMapYear}년)` : `${e[1]} 위치 (좌 ${Y1} · 우 ${Y2})`;
  let html = `<div style="border-top:1px solid var(--line);margin-top:7px;padding-top:6px">
    <div class="leg-row"><div class="leg-dot" style="background:${SEC[e[2]].color};border-radius:50%"></div><span>${posLabel}</span></div>`;
  if (svcOn) {
  ...
  if (gridLayerOn) {
    ...
    if (gridColorMode === 'pop') {
      ...
      gl = `<div class="leg-title">충족 점수 (1~20)</div>
        ...
        <div style="font-size:.66rem;color:var(--ink-3);margin-top:4px">${changeView ? `마우스를 올리면 ${Y1}·${Y2} 비교 표시` : `${detailMapYear}년 격자`}</div>`;
    }
