...
<script src="access_grids.js"></script>
...
<div id="tab-detail" class="tab-pane">
  <div class="detail-tab-layout">
    ...
    <div class="region-main">
      <div style="margin-bottom:14px;min-height:24px">
      ...
      <div id="detail-content" style="display:none">
        ...
        <div class="card card-p" style="margin-bottom:14px">
          <div class="card-h">연도별 추이 <span style="font-weight:400;color:var(--ink-3);font-size:.72rem">(연도별 T점수 — 전국 내 상대적 위치)</span></div>
...
function renderOverview() {
  ...
  const regionCard = (label, rec, color) => {
    ...
    return `<div class="stat-card">
      ...
      <div class="stat-sub" style="color:${color}">${Y2}년 ${f1(rec.infra_idx)}점 ${deltaHtml(rec.infra_idx - v1)}</div>
      <div class="stat-sub" style="margin-top:2px">${Y1}년 ${f1(v1)}점 · ${r1}위 → ${r2}위 ${rankDeltaHtml(r1, r2)}</div>
    </div>`;
  };

  const upCard = bestUp ? `<div class="stat-card">
      <div class="stat-label">최대 상승 🚀</div>
      <div class="stat-val" style="font-size:1rem">${bestUp.sido} ${bestUp.sgg}</div>
      <div class="stat-sub" style="color:#DC2626">${bestUp.rank1}위 → ${bestUp.rank2}위 ${rankDeltaHtml(bestUp.rank1, bestUp.rank2)}</div>
      <div class="stat-sub" style="margin-top:2px">${f1(bestUp.v1)} → ${f1(bestUp.v2)}점 ${deltaHtml(bestUp.dv)}</div>
    </div>` : '';
  const downCard = worstDown ? `<div class="stat-card">
      <div class="stat-label">최대 하락 📉</div>
      <div class="stat-val" style="font-size:1rem">${worstDown.sido} ${worstDown.sgg}</div>
      <div class="stat-sub" style="color:#1D4ED8">${worstDown.rank1}위 → ${worstDown.rank2}위 ${rankDeltaHtml(worstDown.rank1, worstDown.rank2)}</div>
      <div class="stat-sub" style="margin-top:2px">${f1(worstDown.v1)} → ${f1(worstDown.v2)}점 ${deltaHtml(worstDown.dv)}</div>
    </div>` : '';
  ...
  // Sector cards — Y1→Y2 평균 이동 + 대표 지역(최고·최저)의 Δ, 순위변화 1위
  document.getElementById('sector-cards').innerHTML = Object.entries(SEC).map(([k, s]) => {
    ...
    const regionRow = (icon, rec) => {
      const d = rec[key] - (valOf(Y1, rec.sgg_cd, key) ?? rec[key]);
      return `<div style="display:flex;justify-content:space-between;align-items:center">
        <span style="color:#94A3B8;white-space:nowrap">${icon}</span>
        <span style="text-align:right"><strong style="color:#1E293B">${rec.sgg_nm_k}</strong>
          <span style="color:${s.color};font-weight:600">${f1(rec[key])}</span> ${deltaHtml(d)}</span>
      </div>`;
    };
    return `<div class="sector-card">
      <span class="sector-chip" style="background:${s.color}">${s.label}</span>
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin:10px 0 8px">
        <span style="font-size:.7rem;color:#94A3B8"><span class="yr1-chip">${Y1}</span>→<span class="yr2-chip">${Y2}</span></span>
        <span style="font-size:.72rem;color:#64748B">전국범위 ${f1(sortedRecs[sortedRecs.length-1][key])}~${f1(sortedRecs[0][key])}</span>
      </div>
      <div style="font-size:.7rem;border-top:1px solid #F1F5F9;padding-top:6px;display:flex;flex-direction:column;gap:4px">
        ${regionRow('🥇 최고', topRec)}
        ${regionRow('⬇ 최저', botRec)}
        ${secUp ? `<div style="display:flex;justify-content:space-between;align-items:center">
          <span style="color:#94A3B8;white-space:nowrap">🚀 상승1위</span>
          <span style="text-align:right"><strong style="color:#1E293B">${secUp.sgg}</strong>
            <span style="color:#64748B">${secUp.rank1}→${secUp.rank2}위</span> ${rankDeltaHtml(secUp.rank1, secUp.rank2)}</span>
        </div>` : ''}
      </div>
    </div>`;
  }).join('');

  // Top / Bottom 10 — Y1·Y2·Δ·순위변화 병기
  function miniTable(rows) {
    return `<table class="mini-table">
      <thead><tr><th>#</th><th>시도</th><th>시군구</th><th>${Y1}</th><th>${Y2}</th><th>Δ</th><th>순위변화</th></tr></thead>
      <tbody>${rows.slice(0, 10).map((r, i) => {
        ...
        return `<tr>
          ...
          <td style="color:#64748B">${f1(v1)}</td>
          ...
          <td style="white-space:nowrap;color:#64748B">${r1 != null ? r1 + '→' + r2 + '위 ' : ''}${rankDeltaHtml(r1, r2)}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
  }
...
function buildMapLayers(year) {
  const feats = curSidoFilter
    ...
    : GEOJSON.features;
  const showGrid = gridLayerOn && year === Y2;   // 격자는 최신연도 지도에만 (시계열 아님)
  const layers = [
    makeTileLayer(),
...
// TAB 3 : 시군구 상세
// ══════════════════════════════════════════════════════════════════════════════
let radarChart = null, barChart = null, trendChart = null;
...
function renderDetail(rec) {
  ...
  // 직전 연도 대비 순위 변화 배지
  const yi = YEAR_LIST.indexOf(CUR_YEAR);
  const prevY = yi > 0 ? YEAR_LIST[yi - 1] : null;
  let rankDelta = '';
  if (prevY) {
    const prevRank = rankOf(prevY, 'infra_idx', rec.sgg_cd);
    if (prevRank) {
      const d = prevRank - rank;   // +면 순위 상승
      const cls = d > 0 ? 'rank-delta-up' : d < 0 ? 'rank-delta-down' : 'rank-delta-flat';
      const arrow = d > 0 ? `▲${d}` : d < 0 ? `▼${-d}` : '—';
      rankDelta = ` | ${prevY}년 ${prevRank}위 → <span class="${cls}">${arrow}</span>`;
    }
  }
  document.getElementById('detail-rank-badge').style.display = 'inline-block';
  document.getElementById('detail-rank-badge').innerHTML =
    `${rec.sido_nm_k} ${rec.sgg_nm_k} · ${CUR_YEAR}년 종합지수 ${f1(rec.infra_idx)}점 | 전국 ${rank}위 / ${RECORDS.length} (상위 ${pct}%)${rankDelta}`;
  ...
  const convOf = y => radarKeys.map(k => +((valOf(y, rec.sgg_cd, k + '_conv') ?? 0)).toFixed(3));
  const conv1 = convOf(Y1), conv2 = convOf(Y2);
  ...
  // 레이더 축 범위: 실제 그려지는 값(두 연도 + 시도평균)에 타이트하게 + 여백 35%
  const drawnVals = [...conv1, ...conv2, ...sidoAvg];
  ...
  // Radar — 선택 지역 Y1(점선) vs Y2(실선) + 시도 평균(최신)
  if (!radarChart) radarChart = echarts.init(document.getElementById('radar-chart'));
  radarChart.setOption({
    tooltip: { trigger: 'item' },
    legend: { data: [`${Y1}년`, `${Y2}년`, `${rec.sido_nm_k} 평균(${Y2})`], bottom: 0, textStyle: { fontSize: 11 } },
    radar: {
    ...
    series: [{
      type: 'radar',
      data: [
        { value: conv1, name: `${Y1}년`,
          lineStyle: { color: '#94A3B8', width: 1.5, type: 'dashed' }, itemStyle: { color: '#94A3B8' }, areaStyle: { color: 'rgba(148,163,184,.10)' } },
        { value: conv2, name: `${Y2}년`,
          lineStyle: { color: '#2563EB', width: 2.5 }, itemStyle: { color: '#2563EB' }, areaStyle: { color: 'rgba(37,99,235,.18)' } },
        { value: sidoAvg, name: `${rec.sido_nm_k} 평균(${Y2})`,
          lineStyle: { color: '#F59E0B', width: 1.2, type: 'dotted' }, itemStyle: { color: '#F59E0B' } },
      ]
    }]
  }, true);

  // Bar — 부문×세부지수, 연도쌍 (Y1 연한색 · Y2 진한색)
  ...
  barChart.setOption({
    ...
    legend: {
      data: dims.flatMap(d => [`${d.label} ${Y1}`, `${d.label} ${Y2}`]),
      bottom: 0, textStyle: { fontSize: 9 }, itemWidth: 10, itemHeight: 8,
    },
    xAxis: { data: secLabels, axisLabel: { interval: 0, fontSize: 11 } },
    yAxis: { type: 'value', axisLabel: { fontSize: 10 }, scale: true },
    grid: { bottom: 44 },
    series: dims.flatMap(d => [
      { name: `${d.label} ${Y1}`, type: 'bar', barGap: '0%', barCategoryGap: '28%',
        itemStyle: { color: d.color, opacity: .38 },
        data: secKeys.map(k => +((valOf(Y1, rec.sgg_cd, k + '_' + d.sfx) ?? 0)).toFixed(3)) },
      { name: `${d.label} ${Y2}`, type: 'bar',
        itemStyle: { color: d.color },
        data: secKeys.map(k => +((valOf(Y2, rec.sgg_cd, k + '_' + d.sfx) ?? 0)).toFixed(3)) },
    ])
  }, true);

  // Detail table — 연도쌍 + Δ + 순위변화
  const tableRows = secKeys.map(k => {
    const s = SEC[k];
    const key = k + '_conv';
    const cv1 = valOf(Y1, rec.sgg_cd, key);
    const cv2 = rec[key];
    const rk1 = rankOf(Y1, key, rec.sgg_cd);
    const rk2 = natRank(key, cv2);
    const p = upperPct(key, cv2);
    const [mn, mx] = getRange(key);
    const c = colorFor(cv2, mn, mx);
    const subCell = sfx => {
      const a = valOf(Y1, rec.sgg_cd, k + '_' + sfx), b = valOf(Y2, rec.sgg_cd, k + '_' + sfx);
      return `${f1(b)} <span style="font-size:.7rem">${deltaHtml(a != null && b != null ? b - a : null)}</span>`;
    };
    return `<tr>
      <td><span class="sector-chip" style="background:${s.color}">${s.label}</span></td>
      <td style="color:#64748B">${f1(cv1)}</td>
      <td><span class="score-badge" style="background:${c}22;color:${c}">${f1(cv2)}</span></td>
      <td>${deltaHtml(cv1 != null && cv2 != null ? cv2 - cv1 : null)}</td>
      <td style="color:#64748B;font-size:.8rem;white-space:nowrap">${rk2}위 (상위 ${p}%) ${rankDeltaHtml(rk1, rk2)}</td>
      <td>${subCell('sup')}</td>
      <td>${subCell('pop')}</td>
      <td>${subCell('acc')}</td>
    </tr>`;
  ...
  document.getElementById('detail-table').innerHTML = `
    <table class="mini-table">
      <thead><tr>
        <th>부문</th><th>편리성 ${Y1}</th><th>편리성 ${Y2}</th><th>Δ</th><th>전국 순위 (${Y2})</th>
        <th>공급수준</th><th>향유수준</th><th>충족수준</th>
      </tr></thead>
      <tbody>${tableRows}</tbody>
    </table>
    <div style="font-size:.68rem;color:var(--ink-3);margin-top:6px">공급·향유·충족은 ${Y2}년 값 (괄호 Δ = ${Y1} 대비)</div>`;

  // 연도별 추이 (종합 + 5개 부문 편리성)
  if (!trendChart) trendChart = echarts.init(document.getElementById('trend-chart'));
  const trendDefs = [
    { name: '종합', key: 'infra_idx', color: '#1E293B', width: 3 },
    ...secKeys.map(k => ({ name: SEC[k].label, key: k + '_conv', color: SEC[k].color, width: 1.5 }))
  ];
  trendChart.setOption({
    ...
    grid: { top: 22, left: 42, right: 18, bottom: 44 },
    xAxis: { type: 'category', data: YEAR_LIST, boundaryGap: false, axisLabel: { fontSize: 11 } },
    yAxis: { type: 'value', scale: true, axisLabel: { fontSize: 10 } },
    series: trendDefs.map(t => ({
      name: t.name, type: 'line', symbol: 'circle', symbolSize: 7,
      data: YEAR_LIST.map(y => {
        const r = findIn(recsOf(y), rec.sgg_cd);
        return r && r[t.key] != null ? +(+r[t.key]).toFixed(2) : null;
      }),
      lineStyle: { color: t.color, width: t.width },
...
function renderComparison() {
  ...
  const base = cmpSel.length ? RECORDS.find(r => String(r.sgg_cd) === cmpSel[0]) : null;
  ...
  if (cmpMode === 'direct') {
    const b = RECORDS.find(r => String(r.sgg_cd) === cmpSel[1]);
    if (b) compareList = [b];
  } else {
  ...
  // 직접선택(≤2곳)일 때만 연도 오버레이 — 유사 5곳 모드는 최신연도만 (가독성)
  const overlayYears = cmpMode === 'direct';

  const convY = (r, y, k) => valOf(y, r.sgg_cd, k + '_conv');
  // 축 범위: 그려지는 모든 값(오버레이면 두 연도)에 타이트하게 + 여백 35%
  const drawnVals = allRegions.flatMap(r => secKeys.flatMap(k =>
    overlayYears ? [convY(r, Y1, k), convY(r, Y2, k)] : [convY(r, Y2, k)]
  ).filter(v => v != null));
  const _mnC = Math.min(...drawnVals), _mxC = Math.max(...drawnVals);
  const _pad = Math.max((_mxC - _mnC) * 0.35, 3);
  const radarMin = Math.floor(_mnC - _pad);
  ...
  // ── 레이더 차트 ── (오버레이: 지역별 Y1 점선 + Y2 실선)
  const radarData = allRegions.flatMap((r, i) => {
    const y2 = { value: radarKeys.map(k => +(convY(r, Y2, k) ?? 0).toFixed(3)),
                 name: overlayYears ? `${rName(r)} ${Y2}` : rName(r),
                 lineStyle: { color: COLORS[i], width: 2.2 }, itemStyle: { color: COLORS[i] },
                 areaStyle: { opacity: i === 0 ? .18 : .07 } };
    if (!overlayYears) return [y2];
    const y1 = { value: radarKeys.map(k => +(convY(r, Y1, k) ?? 0).toFixed(3)),
                 name: `${rName(r)} ${Y1}`,
                 lineStyle: { color: COLORS[i], width: 1.3, type: 'dashed', opacity: .55 },
                 itemStyle: { color: COLORS[i], opacity: .55 } };
    return [y1, y2];
  });
  ...
  // ── 막대 차트 (5개 부문, 지역×연도 그룹) ──
  if (!cmpBarChart) cmpBarChart = echarts.init(document.getElementById('cmp-bar'));
  const barSeries = allRegions.flatMap((r, i) => {
    const y2 = { name: overlayYears ? `${rName(r)} ${Y2}` : rName(r), type: 'bar',
                 data: secKeys.map(k => +(convY(r, Y2, k) ?? 0).toFixed(1)),
                 itemStyle: { color: COLORS[i] } };
    if (!overlayYears) return [y2];
    const y1 = { name: `${rName(r)} ${Y1}`, type: 'bar', barGap: '0%',
                 data: secKeys.map(k => +(convY(r, Y1, k) ?? 0).toFixed(1)),
                 itemStyle: { color: COLORS[i], opacity: .38 } };
    return [y1, y2];
  });
  cmpBarChart.setOption({
    tooltip: { trigger: 'axis' },
    legend: { data: barSeries.map(s => s.name), bottom: 0, textStyle: { fontSize: 10 }, itemWidth: 10 },
  ...
  // 셀: Y2 값(+기준지역 대비) 위, 아래에 Y1→Y2 자체 변화
  const yearCell = (r, key, baseV2, isBase) => {
    const v1 = valOf(Y1, r.sgg_cd, key), v2 = valOf(Y2, r.sgg_cd, key);
    const baseDiff = isBase || v2 == null || baseV2 == null ? '' : (() => {
      const d = v2 - baseV2;
      const col = d > 0 ? '#16A34A' : d < 0 ? '#DC2626' : '#94A3B8';
      return `<span style="font-size:.72rem;color:${col};margin-left:3px">${d > 0 ? '+' : ''}${d.toFixed(1)}</span>`;
    })();
    return `<td>${f1(v2)}${baseDiff}
      <div style="font-size:.68rem;color:#94A3B8;white-space:nowrap">${Y1}: ${f1(v1)} ${deltaHtml(v1 != null && v2 != null ? v2 - v1 : null)}</div></td>`;
  };

  const sectorRows = secKeys.map(k => {
    const s = SEC[k];
    const key = k + '_conv';
    const baseV2 = valOf(Y2, base.sgg_cd, key);
    const cells = allRegions.map((r, i) => yearCell(r, key, baseV2, i === 0)).join('');
    return `<tr>
      <td><span class="sector-chip" style="background:${s.color}">${s.label}</span></td>
      ${cells}
    </tr>`;
  ...
  const totalRow = (() => {
    const baseV2 = valOf(Y2, base.sgg_cd, 'infra_idx');
    const cells = allRegions.map((r, i) => yearCell(r, 'infra_idx', baseV2, i === 0)).join('');
...
function _toolQueryAccessGrids(input) {
  if (typeof ACCESS_GRIDS === 'undefined') return { error: '격자 데이터(access_grids.js)가 로드되지 않았습니다.' };
  const rec = _findRec(input.sgg_nm);
  if (!rec) return { error: `'${input.sgg_nm || ''}' 시군구를 찾을 수 없습니다.` };
  const grids = ACCESS_GRIDS[rec.sgg_cd] || ACCESS_GRIDS[String(rec.sgg_cd)];
  ...
  return {
    region, grid_count: n,
    note: '0점(전 시설 미충족) 격자는 제외된 수치입니다. 격자 데이터는 최신 시점 기준이며 연도별 시계열이 아닙니다.',
